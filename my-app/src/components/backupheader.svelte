...
<script>
    ...
    let fighter1Img = 0;
    let fighter2Img = 0;
    let fightersData = []; // Declare fightersData here
  
    // Function to highlight the first selected fighter
    storeFighter1.subscribe((fighterId) => {
      fighter1Img = fighterId;
      updateFightersData();
    })
  
    // Function to highlight the second selected fighter
    storeFighter2.subscribe((fighterId) => {
      fighter2Img = fighterId;
      updateFightersData();
    })
  
    const updateFightersData = () => {
      // Extracting data for the selected fighters
      fightersData = dataset
    ...
    onMount(() => {
      ...
      console.log('fighter1Img:', fighter1Img);
      console.log('fighter2Img:', fighter2Img);
    });
  </script>
